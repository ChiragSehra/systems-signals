...
description: "How Cordis combines effects, coeffects, coherence, confluence, and recovery guarantees."
date: 2026-08-17
author: "Chirag Sehra"
categories:
  - Agents
...
The paper combines effect and coeffect information into a unified recursive context model.[@cordiverse2026]
...
The point is that context mutation, recovery and dependency resolution participate in one lifecycle model.
...
## Worked Example: Independent Tool Contributions
...
This resembles Linux supplier/consumer lifecycle ordering, but the paper integrates such ordering into its own dynamic component semantics.[@cordiverse2026; @linuxDeviceLinks]
...
This is why acyclicity assumptions matter when reasoning about eventual quiescence.
...
That is the intuition behind confluence in this setting.
...
## Why Confluence Matters for Agent Runtimes
...
## OS Analogy: Reboot Versus Correct Hot Reconfiguration
...
That is a useful intuition for confluence.

But an operating system as a whole is not thereby formally confluent. The analogy is about the engineering goal.

---

## From Paper to Cordis
...
## DeepSeek Harness Runtime Invariants

DeepSeek Harness also contains a useful complementary idea: package-owned runtime invariants.

Its architecture notes describe `dsh-invariants` as a service that manages invariant registration, child-fiber lifecycle, rollback, disposal and package-attributed failures. Individual packages install checks over their relevant mutable state or event protocols.[@deepseekInvariants2026]
...
- LLM stream grammar;
- monotonic tool execution stages;
- immutable final tool execution/result snapshots;
- durable goal revision and provenance relationships.[@deepseekInvariants2026]

This is not the same thing as the paper's formal metatheory.

However, it points toward an important practical lesson:

> Dynamic composition benefits from both lifecycle semantics **and runtime invariants that detect when the implementation violates its intended contracts**.
...
## What an AI Engineer Should Take Away
...
## Final Discussion: If Rollback Erases Effects, How Does the System Learn?
...
## Runtime Reversibility and Learning Want Different Kinds of History

For composability, we want unwanted runtime history to disappear.

For learning, we want useful epistemic history to survive.

That means a self-improving runtime should probably not model its total state as a single undifferentiated $\Gamma$.
...
These states intentionally have different lifecycle semantics.
...
## The Important Consequence: Full-State Confluence and Learning Are in Tension
...
> **Structural confluence, with intentional epistemic history.**
...
An ephemeral component should not directly make its own temporary state permanent.
...
Once committed, the knowledge record is no longer owned by the ephemeral plugin.

This is how learning can survive rollback without simply becoming untracked residue.

---

## But Persistent Memory Alone Does Not Give a Guarantee

This is the second important point in your question.
...
> **Rollback of executable state without provenance-aware rollback or quarantine of learned state can preserve the consequences of the very component you intended to remove.**

That is a major security consideration for self-editing harnesses.

---

## Learning Needs Provenance
...
Now a rollback can reason about causal history.
...
## Epistemic rollback or quarantine

Find durable knowledge whose provenance depends on the bad runtime version and prevent it from influencing future behaviour until revalidated.
...
The second system can be operationally reverted while semantically remaining compromised.

---

## If Rollback Logs Are Part of the Security Model, They Must Be Outside the Agent's Trust Boundary
...
The record must exist before the risky transition is allowed to become authoritative.
...
This gives recoverability and provenance.
...
## 5. Runtime invariants and post-deploy health checks
...
DeepSeek Harness already has an architecture for package-owned runtime invariant companions over mutable state and event protocols.[@deepseekInvariants2026]
...
## Guarantee 2: Learning persistence
...
## Guarantee 3: Provenance integrity
...
If provenance cannot be verified, the memory should not be considered fully trusted.

---

## Guarantee 4: Audit monotonicity
...
## Guarantee 6: Rollback target immutability

A rollback should restore an immutable known-good artifact.
...
## Guarantee 7: Atomic publication
...
This resembles a publication boundary in session preparation: an exact prepared Session is owned privately until successful publication or rollback, with revision validation preventing stale state from being associated with a newer durable revision.

The same style of thinking can be applied to self-edits.

---

## A Stronger Architecture for a Self-Learning Harness
...
<svg viewBox="0 0 820 500" xmlns="http://www.w3.org/2000/svg" aria-labelledby="part-five-planes-title part-five-planes-desc">
  ...
  <text class="diagram-muted" x="120" y="396">validated memory · provenance · revocable learning</text>
...
│ provenance-aware, revocable/quarantinable   │
└─────────────────────────────────────────────┘
```
...
epistemic → what knowledge should survive my lifetime?
security  → who is allowed to decide what survives?
```

That suggests a useful future extension:

> **Spatiotemporal composability tells us how software components can come and go cleanly. A self-learning harness additionally needs a theory of authorised persistent knowledge.**

The important property would not be full confluence over all state.
...
Every learned state transition is attributable to an immutable source identity and evidence trail.
...
Together, those properties begin to answer the concern that simple persistent memory cannot.

---

## A Final Worked Scenario
...
A provenance-aware system can decide based on causal trust.
